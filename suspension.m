...
Wd = sqrt(eigval) * sqrt ([1 1; 1 1] - (zeta^2));
...
underdamped = exp(-sqrt(eigval) * t_span) .* (q_init(1) * cos(Wd(1) * t_span) + () + sin(Wd(2) * t))
+ ((q_init(:,2) + zeta * sqrt(eigval) * q_init)/Wd) * sin(Wd * t_span))];
